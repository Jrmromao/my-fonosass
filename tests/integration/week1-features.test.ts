...
test.describe('Week 1 MVP Features - Integration Tests', () => {
  ...
  test('should display landing page and allow navigation to dashboard', async ({
    page,
  }) => {
    // Check landing page loads
    await expect(page).toHaveTitle(/Almanaque da Fala/);
  ...
  test('should display exercises in dashboard games page', async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="activities-table"]')
    ).toBeVisible();
  ...
  test('should show exercise preview when clicking actions', async ({
    page,
  }) => {
    await page.goto('http://localhost:3000/dashboard/games');
...
test.describe('Week 1 API Integration Tests', () => {
  ...
  test('should filter exercises by query parameters', async ({ request }) => {
    const response = await request.get(
      'http://localhost:3000/api/exercises?search=Fonema'
    );
