import React, {useCallback, useEffect, useRef, useState, useTransition, useMemo} from 'react';
...
const BalloonField: React.FC<BalloonFieldProps> = ({
    ...
    const memoizedConstants = useMemo(() => ({
        ...
        totalBalloons: 20,
        safePhonemes: phonemes.length >= 20 ? phonemes : [...phonemes, ...phonemes].slice(0, 20),
        heightSections: 4,
    }), []);
    ...
    const checkOverlap = useCallback((balloon1: Balloon, balloon2: Balloon): boolean => {
        ...
        while (!isValid && attempts < maxAttempts) {
            ...
            for (let gx = gridX - 1; gx <= gridX + 1; gx++) {
                for (let gy = gridY - 1; gy <= gridY + 1; gy++) {
                    const gridKey = `${gx},${gy}`;
                    const nearbyBalloons = hitDetectionGridRef.current.get(gridKey) || [];
                    ...
                    for (const balloon of nearbyBalloons) {
                        ...
                        if (distanceSquared < 10000) { // Only check close balloons
                            const tempBalloon: Balloon = {
                                id: -1, x, y, color: "", size: 0.7 + Math.random() * 0.5,
                                popped: false, floatPhase: 0, floatSpeed: 0, floatAmount: 0,
                                rotation: 0, stringLength: 0, hovering: false, pressing: false,
                                anchorGroup: 'left', phoneme: '', zIndex: 0, isDragging: false
                            };
    ...
    const initializeBalloons = useCallback((): void => {
        ...
        const { leftBalloonCount, rightBalloonCount, safePhonemes, heightSections } = memoizedConstants;
        ...
        const createBalloonGroup = (
            ...
            for (let i = 0; i < count; i++) {
                ...
                const position = findValidPosition(balloons, widthRange, heightRange, 25);

                const balloon: Balloon = {
                    id, x: position.x, y: position.y,
                    color: balloonColors[colorIndex], size, popped: false,
                    floatPhase: Math.random() * Math.PI * 2,
                    floatSpeed: 0.3 + Math.random() * 0.3,
                    floatAmount: 2 + Math.random() * 3,
                    rotation: (Math.random() - 0.5) * 0.2,
                    stringLength: Math.sqrt(
                        Math.pow(position.x - anchorX, 2) + Math.pow(position.y - anchorY, 2)
                    ),
                    hovering: false, pressing: false,
                    anchorGroup: isLeft ? 'left' : 'right',
                    phoneme, zIndex: 0, isDragging: false
                };

                balloons.push(balloon);
                
                // Add to spatial grid
                const gridX = Math.floor(position.x / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
                const gridY = Math.floor(position.y / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
    ...
    const popBalloon = useCallback((balloonId: number): void => {
        ...
        const gridX = Math.floor(balloon.x / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
        const gridY = Math.floor(balloon.y / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
        const gridKey = `${gridX},${gridY}`;
        ...
        const fragmentCount = Math.min(20, window.navigator.hardwareConcurrency > 4 ? 20 : 10);
        const fragments: Fragment[] = [];
        const { x, y, color, size } = balloon;

        // Generate fragments in batches for better performance
        const fragmentTypes = [
            { type: 'rubber' as const, count: Math.min(6, fragmentCount / 4), sizeRange: [15, 30], speedRange: [3, 11] },
            { type: 'rubber' as const, count: Math.min(8, fragmentCount / 3), sizeRange: [6, 16], speedRange: [4, 14] },
            { type: 'dust' as const, count: Math.min(15, fragmentCount / 2), sizeRange: [1, 4], speedRange: [5, 17] }
        ];
        ...
        fragmentTypes.forEach(({ type, count, sizeRange, speedRange }) => {
            for (let i = 0; i < count; i++) {
                ...
                const fragmentSize = sizeRange[0] * size + Math.random() * (sizeRange[1] - sizeRange[0]) * size;
    ...
    const findTopmostBalloon = useCallback((x: number, y: number): Balloon | null => {
        const gridX = Math.floor(x / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
        const gridY = Math.floor(y / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
        ...
        return hoveringBalloons.reduce((top, current) =>
            current.zIndex > top.zIndex || (current.zIndex === top.zIndex && current.id > top.id) ? current : top
        );
    }, []);
    ...
    const handleMouseMove = useCallback((e: React.MouseEvent<HTMLCanvasElement>): void => {
        ...
        if (draggingBalloonId !== null) {
            ...
            const balloonIndex = balloonsRef.current.findIndex(b => b.id === draggingBalloonId);
            if (balloonIndex !== -1) {
            ...
            const gridX = Math.floor(mouseX / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
            const gridY = Math.floor(mouseY / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
    ...
    const handleMouseUp = useCallback((e: React.MouseEvent<HTMLCanvasElement>): void => {
        balloonsRef.current = balloonsRef.current.map(balloon => ({
            ...balloon,
            isDragging: false,
            pressing: false
    ...
    const createGradient = useCallback((ctx: CanvasRenderingContext2D, color: string, width: number, height: number): CanvasGradient => {
    ...
    const drawBalloon = useCallback((
        ...
        if (x < -100 || x > ctx.canvas.width + 100 || y < -100 || y > ctx.canvas.height + 100) {
            return;
        }

        let scaleX = 1, scaleY = 1;
        if (hovering) { scaleX = 1.08; scaleY = 1.08; }
        if (pressing) { scaleX = 1.1; scaleY = 0.98; }
        ...
        // Highlights
        ctx.beginPath();
        ctx.ellipse(-balloonWidth / 5, -balloonHeight / 4, balloonWidth / 10, balloonHeight / 8, Math.PI / 4, 0, Math.PI * 2);
        ctx.fillStyle = 'rgba(255, 255, 255, 0.6)';
        ctx.fill();

        ctx.beginPath();
        ctx.ellipse(-balloonWidth / 10, -balloonHeight / 10, balloonWidth / 25, balloonHeight / 25, 0, 0, Math.PI * 2);
    ...
    const startAnimation = useCallback((): void => {
        ...
        const animate = (timestamp: number): void => {
            ...
            const updateCount = Math.min(sortedBalloons.length, PERFORMANCE_CONFIG.BALLOON_UPDATE_BATCH_SIZE);
            ...
            sortedBalloons.forEach((balloon, index) => {
                ...
                if (balloon.x < -100 || balloon.x > width + 100 || balloon.y + floatY < -150 || balloon.y + floatY > height + 50) {
                    return;
                }
            ...
            // Periodic cleanup
            if (timestamp % 60 < 16.66 && fragmentsRef.current.length > PERFORMANCE_CONFIG.MAX_FRAGMENTS_TOTAL) {
    ...
    useEffect(() => {
        const cleanupInterval = setInterval(() => {
            if (fragmentsRef.current.length > PERFORMANCE_CONFIG.MAX_FRAGMENTS_TOTAL) {
        ...
        if (!dialogOpen && poppedBalloonId !== null) {
            ...
            balloonsRef.current = balloonsRef.current.map(balloon => {
                if (balloon.id === poppedBalloonId) {
                    ...
                    const gridX = Math.floor(updatedBalloon.x / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
                    const gridY = Math.floor(updatedBalloon.y / PERFORMANCE_CONFIG.HIT_DETECTION_GRID_SIZE);
                    const gridKey = `${gridX},${gridY}`;
                    if (!hitDetectionGridRef.current.has(gridKey)) {
                        hitDetectionGridRef.current.set(gridKey, []);
                    }
