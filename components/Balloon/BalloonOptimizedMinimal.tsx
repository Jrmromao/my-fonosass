...
const BalloonField: React.FC<BalloonFieldProps> = ({
    ...
    useEffect(() => {
        ...
        if (isMobile) {
            console.log('Mobile device detected - optimizing for touch interaction');
        ...
        // Debug: Log balloon positions after initialization
        setTimeout(() => {
            console.log('=== BALLOON POSITIONS ===');
            balloonsRef.current.forEach(balloon => {
                if (balloon.x > 300 && balloon.y < 150) {
                    console.log(`Top-right balloon ${balloon.phoneme}: x=${balloon.x.toFixed(1)}, y=${balloon.y.toFixed(1)}, size=${balloon.size.toFixed(2)}, popped=${balloon.popped}`);
                }
            });
            console.log('=======================');
        }, 1000);
        ...
        const handleResize = () => {
            console.log('Window resized - reinitializing balloons with new sizes');
            initializeBalloons();
        };
    ...
    // Check if two balloons overlap - ensure they never overlap
    const checkOverlap = (balloon1: Balloon, balloon2: Balloon): boolean => {
        ...
        // Responsive sizing for overlap detection
        ...
        let baseSize;
        if (isLargeScreen) {
            baseSize = 50; // Larger base size for large screens
        } else if (isMediumScreen) {
            baseSize = 45; // Medium size for tablets
        } else {
            baseSize = isMobile ? 40 : 35; // Mobile-optimized sizing
        }
        
        const size1 = baseSize * balloon1.size;
        const size2 = baseSize * balloon2.size;

        // No overlap allowed - balloons must be completely separate
        const minDistance = (size1 + size2) * 0.8; // 80% separation to ensure no overlap
    ...
    const initializeBalloons = (): void => {
        ...
        const createBalloonGroup = (
            ...
            for (let i = 0; i < count; i++) {
                ...
                if (isLargeScreen) {
                    baseSize = 0.8; // Larger base size for large screens
                    sizeVariation = 0.4; // More variation: 0.8 to 1.2
                } else if (isMediumScreen) {
                    baseSize = 0.7; // Medium size for tablets
                    sizeVariation = 0.3; // 0.7 to 1.0
                } else {
                    baseSize = 0.6; // Smaller size for mobile
                    sizeVariation = 0.3; // 0.6 to 0.9
                }
                
                const size = baseSize + Math.random() * sizeVariation;
                const phoneme = safePhonemes[id % safePhonemes.length];

                        // Find a valid position with more attempts to ensure no overlap
        const position = findValidPosition(balloons, widthRange, heightRange, size, 100); // More attempts to find non-overlapping position
        ...
        // Debug: Log balloon positions, especially for top-right area
        console.log(`[BALLOON INIT] Created ${balloons.length} balloons`);
        balloons.forEach(balloon => {
            if (balloon.x > 300 && balloon.y < 150) {
                console.log(`[BALLOON INIT] Balloon ${balloon.phoneme} (ID: ${balloon.id}) at (${balloon.x.toFixed(1)}, ${balloon.y.toFixed(1)}) - size: ${balloon.size.toFixed(2)}`);
            }
        });
    };
    ...
    const findValidPosition = (
        ...
        while (!isValid && attempts < maxAttempts) {
            ...
            for (const balloon of balloons) {
                ...
                // Create a temporary balloon to check overlap with the EXACT size being created
                const tempBalloon: Balloon = {
                    id: -1,
                    x,
                    y,
                    color: "",
                    size: balloonSize, // Use the exact size of the balloon being created
                    popped: false,
                    floatPhase: 0,
                    floatSpeed: 0,
                    floatAmount: 0,
    ...
    const startAnimation = (): void => {
        ...
        const animate = (timestamp: number): void => {
            ...
            // Performance optimization: Only update some balloons each frame
    ...
    const findTopmostBalloon = (x: number, y: number): Balloon | null => {
        const hoveringBalloons = balloonsRef.current.filter(balloon => {
            ...
            let baseHitRadius;
            if (isLargeScreen) {
                baseHitRadius = 70; // Larger hit radius for large screens
            } else if (isMediumScreen) {
                baseHitRadius = isMobile ? 80 : 60; // Medium hit radius for tablets
            } else {
                baseHitRadius = isMobile ? 80 : 50; // Mobile-optimized hit radius
            }
            const hitRadius = baseHitRadius * balloon.size;

            // Enhanced debug logging for problematic area (top-right corner)
            if (x > 300 && y < 150) {
                const floatingY = balloon.y + Math.sin(balloon.floatPhase) * balloon.floatAmount;
                console.log(`[HIT DETECTION] Balloon ${balloon.phoneme}: baseX=${balloon.x.toFixed(1)}, baseY=${balloon.y.toFixed(1)}, floatY=${floatingY.toFixed(1)}, clickX=${x.toFixed(1)}, clickY=${y.toFixed(1)}, distance=${distance.toFixed(1)}, hitRadius=${hitRadius.toFixed(1)}, size=${balloon.size.toFixed(2)}`);
            }
        ...
        if (hoveringBalloons.length === 0) {
            if (x > 300 && y < 150) {
                console.log(`[HIT DETECTION] No balloons found at (${x.toFixed(1)}, ${y.toFixed(1)})`);
            }
            return null;
        }
        ...
        if (x > 300 && y < 150) {
            console.log(`[HIT DETECTION] Selected balloon: ${selected.phoneme} (ID: ${selected.id}) from ${hoveringBalloons.length} candidates`);
        }
    ...
    const handlePointerDown = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>): void => {
        ...
        // Enhanced mobile debugging for problematic area
        if (clickX > 300 && clickY < 150) {
            const isMobile = 'ontouchstart' in window || navigator.maxTouchPoints > 0;
            console.log(`[${isMobile ? 'MOBILE' : 'DESKTOP'}] Click in top-right area: (${clickX.toFixed(1)}, ${clickY.toFixed(1)})`);
            console.log(`Canvas: ${canvas.width}x${canvas.height}, Rect: ${rect.width}x${rect.height}`);
            console.log(`Scale factors: X=${(canvas.width / rect.width).toFixed(2)}, Y=${(canvas.height / rect.height).toFixed(2)}`);
            if ('touches' in e) {
                console.log(`Touch event: ${e.touches.length} touches`);
            }
        }

        const topmostBalloon = findTopmostBalloon(clickX, clickY);

        // Debug logging for clicks in problematic area
        if (clickX > 300 && clickY < 150) {
            console.log(`[CLICK HANDLER] Click at (${clickX.toFixed(1)}, ${clickY.toFixed(1)}) - Found balloon: ${topmostBalloon ? topmostBalloon.phoneme : 'NONE'}`);
        }
    ...
    const throttledPointerMove = useCallback(
        (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>): void => {
            const canvas = canvasRef.current;
            if (!canvas) return;
    ...
    const handlePointerUp = (e: React.MouseEvent<HTMLCanvasElement> | React.TouchEvent<HTMLCanvasElement>): void => {
        balloonsRef.current = balloonsRef.current.map(balloon => ({
            ...balloon,
            isDragging: false,
            pressing: false
