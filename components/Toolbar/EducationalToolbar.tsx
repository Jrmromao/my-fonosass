...
const EducationalToolbar: React.FC = () => {
    ...
    // Create a reference to store the bubbles
    const bubblesRef = useRef<React.ReactNode[]>([]);
    ...
    // Floating bubble animations (for playful background)
    const generateBubbles = (): React.ReactNode[] => {
        if (bubblesRef.current.length > 0) return bubblesRef.current;

        const bubbles: React.ReactNode[] = [];
        // Limit number of bubbles to improve performance
        for (let i = 0; i < 10; i++) {
            ...
            >
                <button
                    ...
                    style={{
                        ...
                        border: isActive ? `3px solid ${category.color}` : isHovered ? `2px solid ${category.color}` : `2px solid transparent`,
                        zIndex: 10
                    }}
                >
                    {/* Fun background for children */}
                    <div
                        className={`absolute inset-0 opacity-10 transition-opacity duration-200 ${isActive || isHovered ? 'opacity-20' : 'opacity-0'}`}
                    ...
                    <motion.div
                        ...
                        style={{
                            backgroundColor: isActive ? 'white' : isHovered ? `${category.bgColor}` : 'transparent',
    ...
    return (
        <div className="relative rounded-2xl overflow-hidden shadow-lg border-4 border-purple-200 dark:border-purple-800 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-900 dark:to-purple-900">
            ...
            <div className="absolute inset-0 border-8 border-transparent rounded-xl z-5"
                 style={{
