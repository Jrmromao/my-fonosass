...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.log('Waiting list entry saved locally:', newEntry)
    } catch (localError) {
      console.log('Local file save failed (expected in production):', localError instanceof Error ? localError.message : String(localError))
    ...
    try {
      ...
      await s3Service.uploadFile(individualEntryKey, individualEntry, 'application/json')
      ...
      console.log('Waiting list data uploaded to S3 successfully')
    } catch (s3Error) {
